...
function randomizeInstruments() {
  for(let i = 0; i < 16; i++) {
    // Pula o canal de percussão (padrão General MIDI) para não perder o ritmo base
    if(i === 9) continue; 
    
    // Sorteia um número entre 0 e 127
    const randomProg = Math.floor(Math.random() * 128);
    
    // Aplica a mudança no motor de áudio
    changeChannelProgram(i, randomProg);
    
    // Atualiza o select visualmente na interface
    const selectEl = document.getElementById(`ch-prog-${i}`);
    if(selectEl) {
      selectEl.value = randomProg;
    }
  }
  
  showSaveToast('🎲 Instrumentos randomizados!');
  console.log('[DJ MID] Arranjo gerado proceduralmente!');
}
